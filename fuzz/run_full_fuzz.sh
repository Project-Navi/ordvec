...
# All cores but two, so the machine stays responsive and a small box is not
# pegged. Use FORKS=$(nproc) for every core. Peak RAM ~= FORKS x RSS_LIMIT_MB.
NCPU="$(nproc)"
...
for t in ${TARGETS}; do
  ...
  cargo +nightly fuzz run "${t}" -- \
      ...
      -print_final_stats=1
  echo "### target ${t} finished $(date -Is) (libfuzzer rc=$?)"
  echo
done

echo "============================================================"
echo "=== campaign complete $(date -Is) — crash artifact summary ==="
crashes=$(find fuzz/artifacts -type f \
  \( -name 'crash-*' -o -name 'oom-*' -o -name 'timeout-*' -o -name 'leak-*' \) 2>/dev/null)
if [ -z "${crashes}" ]; then
  echo "CLEAN: no crash / oom / timeout / leak artifacts across any target."
  status=0
else
