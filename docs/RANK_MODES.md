...
single command, no external data and no system dependencies (ordvec
links no BLAS):
...
is deliberately anisotropic (latent_dim=64 in D=256), which is the
regime that most favours an ordinal view, so treat the synthetic
recall *gaps* between modes as a best case rather than a typical case
(see the [external-corpus section](#external-corpus-results-user-runnable)).
...
which favours an ordinal view — so the recall *spread* between modes
here is a best case, not a typical case. On real embeddings with
milder anisotropy the spread is smaller; the
...
numbers above are from the synthetic headline run; the per-vector cost
is data-independent.
...
check the modes on real embeddings, point the same bench at your own
`.npy` arrays:
...
claim): on dense sentence/passage encoders the recall spread between
modes is **smaller** than on the synthetic anisotropic corpus, and the
best operating point is **encoder- and bit-width-dependent**. The
synthetic corpus is a favourable case for the ordinal view. Run the
command above on your target embeddings to get the number that matters
for your deployment — this repo does not assert a specific real-corpus
delta because the corpus is not shipped here. Real-corpus evaluation
appears in the accompanying paper (link TBD).
...
headline numbers' quality columns are deterministic given the default
...
4. **Recall is corpus-dependent.** On the synthetic anisotropic corpus
   the asymmetric ordinal scan wins by a wide margin over the
   symmetric scan; on real embeddings the spread is smaller and the
   best operating point depends on encoder and bit width (run the
